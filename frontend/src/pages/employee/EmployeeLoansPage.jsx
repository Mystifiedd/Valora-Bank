...
export default function EmployeeLoansPage() {
  ...
  const handleDecision = async (id, status) => {
    setActing(true);
    try {
      await api.patch(`/employee/loans/${id}/decision`, { status });
  ...
  return (
    <div className="stack stack-lg">
      <div className="page-header">
        <h1 className="page-header__title">Loan Management</h1>
      </div>
        ...
        <div className="table-card">
          <div className="table-responsive">
            <table className="table-modern">
              <thead>
                <tr><th>ID</th><th>User</th><th>Type</th><th>Amount</th><th>Interest</th><th>Term</th><th>Status</th><th>Applied</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {loans.length === 0 ? (
                  <EmptyRow colSpan={9} message="No loans found." />
                ) : loans.map((l) => (
                  <tr key={l.id}>
                    <td>{l.id}</td>
                    <td>{l.user_id}</td>
                    <td className="text-capitalize">{l.loan_type}</td>
                    <td style={{ fontWeight: 600 }}>Rs. {formatCurrency(l.principal_amount)}</td>
        ...
        {selected && (
          <div>
            <dl className="detail-list">
              ...
              <dt>Loan Type</dt><dd className="text-capitalize">{selected.loan_type}</dd>
              <dt>Amount</dt><dd>Rs. {formatCurrency(selected.principal_amount)}</dd>
              <dt>Interest Rate</dt><dd>{selected.interest_rate}%</dd>
              <dt>Term</dt><dd>{selected.tenure_months} months</dd>
